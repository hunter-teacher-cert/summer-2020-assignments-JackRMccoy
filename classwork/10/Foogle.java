...
public class indexOfgle
{
  ...
  //  <Search for index of target - int index when equal to target, return -1 if out of bounds>
  ...
  //  <Returns an arraylist - putting something onto the array - filling the array with a certain number random values from a floor to a ceiling>
  public static ArrayList randomBetween (int numItems, int lo, int hi)
  {
  ...
  //  < Adds a random number from lo to hi to the right side of the array - If array empty - return as is, lo is now the ceiling, adds a number from 0 to lo>
  public static ArrayList addRandToPrev(int numItems, int lo, int hi)
  {
    ArrayList retArr = new ArrayList();

    // <if numItems is zero, return empty array>
    if ( numItems<1 )
      return retArr;

    // <begin an array gets a number between 0 and lo>
    retArr.add( (int)(lo * Math.random()) );

    // <make new element equal previous element plus some random number between lo and hi>
    for(int i=1; i<numItems; i++) {
      ...
      retArr.add( (int)retArr.get(i-1)
                  + lo + (int)(hi * Math.random()) ); //previous elemnt
    }
  ...
  {
    // test battery for randomBetween()

    System.out.println("\n"+"al000"+":");
    ArrayList al000 = randomBetween(0,0,0);
    ...
    /*
    System.out.println("\n"+"al"+":");
    ArrayList al = randomBetween(,0,100);
